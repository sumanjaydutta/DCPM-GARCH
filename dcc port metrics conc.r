...
# Step 3: Run Simulation for Different Sample Sizes
set.seed(123)
n <- 500  # 500 stocks
Sigma <- matrix(0.5, n, n) + diag(0.5, n)  # Create a covariance matrix with moderate correlations
weights <- rep(1/n, n)  # Equal weights for the portfolio

sample_sizes <- seq(50, 1000, by = 50)  # Sample sizes from 50 to 1000
results_table <- data.frame(SampleSize = sample_sizes, DCC_Return = NA, DCC_StdDev = NA, DCC_InfoRatio = NA, 
...
for (s in sample_sizes) {
  ...
  results_table[results_table$SampleSize == s, "DCC_Return"] <- dcc_metrics$annualized_return
  results_table[results_table$SampleSize == s, "DCC_StdDev"] <- dcc_metrics$annualized_std_dev
  results_table[results_table$SampleSize == s, "DCC_InfoRatio"] <- dcc_metrics$info_ratio
  
  results_table[results_table$SampleSize == s, "COV_Return"] <- xdcc_metrics$COV$annualized_return
  results_table[results_table$SampleSize == s, "COV_StdDev"] <- xdcc_metrics$COV$annualized_std_dev
  results_table[results_table$SampleSize == s, "COV_InfoRatio"] <- xdcc_metrics
